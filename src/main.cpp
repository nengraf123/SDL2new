...
// Функция для проверки, находится ли точка (x, y) внутри прямоугольника
bool IsPointInRect(int x, int y, const SDL_Rect* rect) {
...
int main(int argc, char* argv[]) {
    // Инициализация SDL с поддержкой видео
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        std::cerr << "SDL initialization failed: " << SDL_GetError() << std::endl;
        return 1;
    }

    // Указываем Wayland как видеодрайвер (опционально, если окружение уже настроено)
    ...
    SDL_Rect square_rect = { 100, 100, 100, 100 }; // x, y, ширина, высота
    bool is_square_present = true;
    bool is_square_hovered = false;

    SDL_Color color_default = { 0, 0, 255, 255 }; // Синий
    SDL_Color color_hovered = { 255, 255, 0, 255 }; // Желтый
    SDL_Color color_background = { 0, 0, 0, 255 }; // Черный фон
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN) {
                if (event.button.button == SDL_BUTTON_LEFT) {
                    printf("Нажата одиночная ЛЕВАЯ кнопка мыши\n");
                    if (is_square_present && IsPointInRect(event.button.x, event.button.y, &square_rect)) {
                        std::cout << "Квадрат нажат, он исчезает!" << std::endl;
                        is_square_present = false;
                        is_square_hovered = false;
                    }
                }
                if (event.button.button == SDL_BUTTON_RIGHT) {
                    printf("Нажата одиночная ПРАВАЯ кнопка мыши\n");
                }
                if (event.button.button == SDL_BUTTON_MIDDLE) {
                    printf("Нажата одиночная СРЕДНЯЯ (колесико) кнопка мыши\n");
                }
            }
            if (event.type == SDL_MOUSEMOTION) {
                if (is_square_present) {
